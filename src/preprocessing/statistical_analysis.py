...
# classes = ['fixed_gear', 'pole_and_line', 'trollers']
...
stats_df = pd.DataFrame(columns=['Class', 
                                'Mean speed (knots)', 'Mean course change', 'Mean distance travelled (miles)', 
                                'std. speed', 'std. course change','std. distance'])


# sub_df = agg_list_df[5] + agg_list_df[2]
...
for idx, class_list in enumerate(agg_list_df):
    ...
    for df in tqdm(class_list):    
        ...
        df['t_hour_pc'] = df['delta_t'].dt.total_seconds() / (60*60)
        if sum(df['t_hour_pc']) == 0: continue
        df['dist'] = df['t_hour_pc'] * df['speed']
        avg_course_change = df['delta_c'].mean()
    ...
    avg_speeds = avg_speeds[np.isfinite(avg_speeds)]
    total_avg_speed = np.mean(avg_speeds)    
    total_std_speed = np.std(avg_speeds)
    
    avg_courses = np.array(avg_courses)
    avg_courses = avg_courses[np.isfinite(avg_courses)]
...
print(stats_df)
# save
stats_df.to_csv('../../data/sa.csv', encoding='utf-8', index=False)
