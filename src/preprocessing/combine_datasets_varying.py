...
match case 'varying':
    ...
    # pole and line
    pole_list_df = pole_module.build_sequences(time_period=24, threshold=1, drop_columns=drop_columns)
    ...
    # purse seines
    purse_list_df = purse_module.build_sequences(time_period=24, threshold=1, drop_columns=drop_columns)
    ...
    # fixed gear
    fixed_list_df = fixed_module.build_sequences(time_period=24, threshold=1, drop_columns=drop_columns)
    ...
    # trawlers
    trawlers_list_df = trawlers_module.build_sequences(time_period=24, threshold=1, drop_columns=drop_columns)
    ...
    # drifting longlines
    drifting_list_df = drifting_module.build_sequences(time_period=24, threshold=1, drop_columns=drop_columns)
    ...
    print(sum(aggregate_mask), ' == ', len(df1))
    
    # save dask dataframe and the mask
...
df_list = module.re_segment(df, mask)
...
total = len(data_train) + len(data_valid) + len(data_test) # only 3 off the original size of the data 




# rebuild time sequences
data_train_seqs = module.re_segment(data_train, mask_train, dataframe=False)
data_valid_seqs = module.re_segment(data_valid, mask_valid, dataframe=False)
data_test_seqs = module.re_segment(data_test, mask_test, dataframe=False)
...
# data_train, data_valid, data_test = module.split(df)

print(sum(pole_mask))
print(sum(troller_mask))

aggregate_df.npartitions
aggregate_df.dtypes
aggregate_df.columns
pandas_df = aggregate_df.compute()
