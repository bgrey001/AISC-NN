...
class GRU_wrapper():
    ...
    # Data attributes
    # =============================================================================
    datatype = 'varying' # being padded in the AIS_loader class
    data_ver = '2'
    shuffle = True
    # =============================================================================
    # Hyperparameters
    ...
    def __init__(self, GRU, optimizer):
        ...
        self.train_data = data_module.AIS_loader(choice=self.datatype, split='train', version=self.data_ver)
        self.valid_data = data_module.AIS_loader(choice=self.datatype, split='valid', version=self.data_ver)
        self.test_data = data_module.AIS_loader(choice=self.datatype, split='test', version=self.data_ver)
    ...
    def fit(self, validate):
        ...
        train_generator = DataLoader(dataset=self.train_data, batch_size=self.batch_size, shuffle=self.shuffle, collate_fn=self.train_data.collate_fn)
        valid_generator = DataLoader(dataset=self.valid_data, batch_size=self.batch_size, shuffle=self.shuffle, collate_fn=self.train_data.collate_fn)
        ...
        for epoch in range(self.epochs):
            ...
            outputs  = torch.zeros(self.batch_size).int().to(self.device)
    ...
    def predict(self):
        ...
        test_generator = DataLoader(dataset=self.test_data, batch_size=self.batch_size, shuffle=self.shuffle, collate_fn=self.train_data.collate_fn)
        test_index = 0
    ...
    def print_summary(self):
        print(f'\nModel: GRU_v{self.version_number} -> Hyperparamters: \n'
              ...
              f'\nData: padded, version {self.data_ver}, varying intervals \nSequence length = {self.seq_length} \nBatch size = {self.batch_size} \n Shuffled = {self.shuffle}'
              )
        
        print(f'=====================================================================================================================\n'
              f'|  Training accuracy  |  Training loss  |  Validation accuracy  |  Validation loss  |  Test accuracy  |  Test loss  |'
              f'=====================================================================================================================\n'
              f'|        {"{:.4f}".format(round(self.history["training_accuracy"][-1], 4))}       |     {"{:.3f}".format(round(self.history["training_loss"][-1], 3))}      |        {"{:.3f}".format(round(self.history["validation_accuracy"][-1], 3))}         |      {"{:.4f}".format(round(self.history["validation_loss"][-1], 4))}       |      {"{:.3f}".format(round(self.history["test_accuracy"][0], 3))}     |    {"{:.4f}".format(round(self.history["test_loss"][0], 4))}   |'
              f'=====================================================================================================================')
